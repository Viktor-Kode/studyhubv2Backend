...
async function run() {
  ...
  for (const u of freeUsers) {
    ...
    console.log(
      `  [FREE] ${u.email} — aiLimit: ${u.aiUsageLimit} → 3 | noteLimit: ${u.noteUsageLimit ?? 'unset'} → 3 | quizLimit: ${u.quizUsageLimit ?? 'unset'} → 3${hasWelcomeCredits ? ' | resetting welcome credits' : ''}`
    );
